...
from .data_utils import read_data, read_jsonl, copy_dev_test
...
def compute_train_dy_metrics(training_dynamics, args):
  ...
  all_list = list(training_dynamics.values())
  max_num=0
  for kk in all_list:
    if max_num<len(kk["logits"]):
      max_num=len(kk["logits"])

  num_tot_epochs = max_num
  print("n\n\n******************* num_tot_epochs:", num_tot_epochs)
  ...
  logits = {i: [] for i in range(num_tot_epochs+2)}
  targets = {i: [] for i in range(num_tot_epochs+2)}
  print("\n\n\n**********logits:", logits)
...
if __name__ == "__main__":
  ...
  if args.plot:
    ...
    #train_dy_metrics.to_json('train_dynamics.jsonl', orient='records', lines=True)
    df_hard = train_dy_metrics[(train_dy_metrics['confidence']>0.7) | (train_dy_metrics['variability']>0.35)]
    #train_dy_metrics.to_json('train_dynamics.jsonl', orient='records', lines=True)
    df_hard.to_json('squad_easy_ambg.jsonl', orient='records', lines=True)
